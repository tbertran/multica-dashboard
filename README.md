...
- Node.js 20 or newer
- The [`multica` CLI](https://multica.ai/docs/cli), installed and signed in. The dashboard
  reuses its saved token (`~/.multica/profiles/<profile>/config.json`) instead of holding a
  second copy.

```sh
multica whoami                         # confirms the CLI profile exists
npm install
...
The server listens on all interfaces so a phone on the same network can open it, and it has
no login of its own: anyone who can reach the port can read your issues and post comments
as you. Only run it on a network you trust.
