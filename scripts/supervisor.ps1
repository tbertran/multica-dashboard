...
while ($true) {
    ...
    $stillStarting = $current -and -not $current.HasExited
    if (-not $listening -and -not $stillStarting) {
        $current = Start-Process -FilePath $node -ArgumentList "src\server.js" -WorkingDirectory $appDir -WindowStyle Hidden -PassThru
    }
