...
export class UpstreamFeed {
  ...
  async connect() {
    ...
    // A socket that dies silently (sleep/wake, NAT drop) never emits 'close',
    // so without a ping/pong watchdog the feed stays "open" and delivers nothing.
    let alive = true;
    let heartbeat = null;
    ...
    ws.on('message', (raw) => {
      ...
      if (msg.type === 'auth_ack') {
        ...
        // Events missed while the socket was down are not replayed; this tells
        // every tab to re-fetch instead of staying stale until the next event.
        for (const fn of this.listeners) fn({ type: 'agent:resync' });
        return;
      }
